...
const game_url_base = "http://www.espn.com/college-football/game?gameId=";
...
var schedule = {
  ...
  find_games: function (html) {
    var $ = cheerio.load(html);
    return $('span.rank').closest("tr").find('[data-date] a').map(function (i, e) {
      var id = $(e).attr('href').match(/gameId=(\d*)/)[1];
      return id;
    }).get();
  },
